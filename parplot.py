...
Originally from: https://github.com/erdc-cm/proteus-notebooks (Public Domain)

parallel_data_plot added to allow plotting intermediates from data_pub.
"""
...
def parallel_plot(view, method, *args, **kwargs):
    ...
    return stitch_images(ar, imshow)


def parallel_data_plot(plot_name, data_list, *arg_names, **kwargs):
    """Like parallel_plot, but give ar.data, data keys instead."""
    import matplotlib.pyplot as plt
    pngs = []
    plot = getattr(plt, plot_name)
    for data in data_list:
        args = [ data[a] for a in arg_names ]
        pngs.append(_plot_f(plot_name, *args, **kwargs))
    return stitch_images(pngs)


def stitch_images(image_container, imshow=False):
    pngiter = iter(image_container)
    merged = next(pngiter)
